...
<div class="buttons">
	{#each items as item}
		<div class="button">
			...
			<div class="edit">
				<sl-button type="default" size="medium" circle on:click={() => edit(item)}
					><sl-icon name="pencil-square" /></sl-button
				>
			</div>

			<sl-button on:click={() => console.log(item)}>{item}</sl-button>
			<sl-button type="default" size="medium" circle on:click={() => edit(item)}>
				<sl-icon name="pencil-square" /></sl-button
			>
		</div>
